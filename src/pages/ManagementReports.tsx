...
const getResolutionEndDate = (ticket: ManagementReportTicket) => ticket.service_finished_at || ticket.resolved_at || ticket.closed_at;

const average = (durations: number[]) => (
  durations.length ? durations.reduce((sum, duration) => sum + duration, 0) / durations.length : null
);

const buildExecutorProductivity = (tickets: ManagementReportTicket[]): ExecutorProductivityRow[] => {
  ...
  tickets.forEach((ticket) => {
    ...
    ticket.executor_details.forEach((executor) => {
      ...
      if (risk.level !== 'warning' && risk.level !== 'critical') current.slaOk += 1;
...
export default function ManagementReports() {
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      if (tipoFilter !== 'all') executorOptionsQuery = executorOptionsQuery.eq('area', tipoFilter);
      ...
      <main className="container space-y-4 px-3 py-3 sm:px-4 sm:py-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Conta cada participação individual. Tickets sem executor permanecem na relação detalhada, mas ficam fora desta produtividade.
            </CardDescription>
          </CardHeader>
